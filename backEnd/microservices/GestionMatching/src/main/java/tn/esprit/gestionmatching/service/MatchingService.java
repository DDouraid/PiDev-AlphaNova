...
public class MatchingService {
    ...
    public List<MatchResponseWithOfferDto> matchCvWithOffers() throws IOException {
        try {
            ...
            // Call Flask API for admission prediction
            AdmissionPredictionDTO admissionPrediction = predictAdmission(cvData);
            System.out.println("Admission prediction: " + admissionPrediction);
            ...
            for (InternshipOffer offer : offers) {
                ...
                MatchResponseWithOfferDto matchResponse = new MatchResponseWithOfferDto(offer.getId(), matchScore, offerDto);
                matchResponse.setAdmissionPrediction(admissionPrediction); // This now works with the correct DTO
                matchResults.add(matchResponse);
            }
    ...
    private AdmissionPredictionDTO predictAdmission(CvDataDto cvData) {
        try {
            // Prepare headers
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(org.springframework.http.MediaType.APPLICATION_JSON);

            // Convert CvDataDto to a Map for JSON serialization
            Map<String, Object> cvDataMap = new HashMap<>();
            cvDataMap.put("userId", cvData.getUserId());
            cvDataMap.put("skills", cvData.getSkills());
            cvDataMap.put("education", cvData.getEducation().stream().map(edu -> {
                Map<String, String> eduMap = new HashMap<>();
                eduMap.put("degree", edu.getDegree());
                eduMap.put("institution", edu.getInstitution());
                eduMap.put("year", edu.getYear());
                return eduMap;
            }).collect(Collectors.toList()));
            cvDataMap.put("certifications", cvData.getCertifications().stream().map(cert -> {
            ...
            cvDataMap.put("greScore", cvData.getGreScore());
            cvDataMap.put("toeflScore", cvData.getToeflScore());
            cvDataMap.put("universityRating", cvData.getUniversityRating());
            cvDataMap.put("sop", cvData.getSop());
            cvDataMap.put("LOR", cvData.getLor()); // Changed from "lor" to "LOR" to match Flask API
            cvDataMap.put("cgpa", cvData.getCgpa());
            cvDataMap.put("research", cvData.getResearch());

            // Add debug logging to verify the payload
            System.out.println("Sending payload to Flask API: " + cvDataMap);

            // Create HTTP entity with the CV data
            HttpEntity<Map<String, Object>> entity = new HttpEntity<>(cvDataMap, headers);

            // Call Flask API
            System.out.println("Calling Flask API at: " + mlServiceUrl + "/predict");
            ResponseEntity<AdmissionPredictionDTO> response = restTemplate.exchange(
                    mlServiceUrl + "/predict",
                    HttpMethod.POST,
                    entity,
                    AdmissionPredictionDTO.class
            );

            AdmissionPredictionDTO prediction = response.getBody();
            if (prediction == null) {
                throw new RuntimeException("Failed to get admission prediction from Flask API");
            }
            return prediction;

        } catch (Exception e) {
            System.err.println("Error calling Flask API for admission prediction: " + e.getMessage());
            e.printStackTrace();
            // Return a default prediction in case of failure
            AdmissionPredictionDTO defaultPrediction = new AdmissionPredictionDTO();
            defaultPrediction.setAdmitted("No");
            defaultPrediction.setProbability(0.0);
            defaultPrediction.setRecommendedUniversity("Unknown");
            defaultPrediction.setCluster(0);
