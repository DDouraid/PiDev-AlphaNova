...
export class MatchingComponent implements OnInit, AfterViewInit {
  ...
  currentStudentId: number = 0; // Should be set based on the logged-in user
  ...
  ngOnInit(): void {
    // Set the current student ID (this should ideally come from an authentication service)
    this.currentStudentId = 6; // Hardcoded for now; replace with actual student ID from auth service
    ...
    this.matchingService.matchCvWithOffers().subscribe({
      next: (matches) => {
        ...
        if (matches.length > 0) {
          ...
          this.selectedOffer = matches[0].offerDetails;
        } else {
  ...
  getMatchScore(offerId: number): number {
    ...
    return match && typeof match.matchScore === 'number' ? Math.round(match.matchScore * 100) : 0;
  }

  getPredictionTitle(probability: number): string {
    if (probability > 0.7) {
  ...
  getAdmissionPrediction(offerId: number) {
    const match = this.matches.find(m => m.offerId === offerId);
    return match?.admissionPrediction;
  }
  ...
  onPredictionComplete(prediction: any) {
    console.log('Prediction result:', prediction);
  }

  selectOffer(match: MatchResult): void {
    this.selectedOffer = match.offerDetails;
  }
}