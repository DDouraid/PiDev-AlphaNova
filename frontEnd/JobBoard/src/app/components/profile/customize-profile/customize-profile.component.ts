...
export class CustomizeProfileComponent implements OnInit {
  ...
  constructor(
    ...
    this.cvForm = this.fb.group({
      ...
      greScore: ['', [Validators.min(0), Validators.max(340)]],
      toeflScore: ['', [Validators.min(0), Validators.max(120)]],
      universityRating: ['', [Validators.min(0), Validators.max(5)]],
      sop: ['', [Validators.min(0), Validators.max(5)]],
      lor: ['', [Validators.required, Validators.min(0), Validators.max(5)]],
      cgpa: ['', [Validators.min(0), Validators.max(4)]],
      research: ['', [Validators.min(0), Validators.max(1)]]
    });
  }
 ...
 downloadCV(): void {
  ...
  private addCVContent(doc: jsPDF, startY: number, formValue: any): Promise<void> {
    return new Promise((resolve) => {
      ...
      // Add Academic Scores section
      if (formValue.greScore || formValue.toeflScore || formValue.universityRating || 
          formValue.sop || formValue.lor || formValue.cgpa || formValue.research) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(14);
        ...
        if (formValue.greScore) {
          doc.text(`GRE Score: ${formValue.greScore}`, 25, yPosition);
          yPosition += 7;
        }
        if (formValue.toeflScore) {
          doc.text(`TOEFL Score: ${formValue.toeflScore}`, 25, yPosition);
          yPosition += 7;
        }
        if (formValue.universityRating) {
          doc.text(`University Rating: ${formValue.universityRating}/5`, 25, yPosition);
          yPosition += 7;
        }
        if (formValue.sop) {
          doc.text(`Statement of Purpose Rating: ${formValue.sop}/5`, 25, yPosition);
          yPosition += 7;
        }
        if (formValue.lor) {
          doc.text(`Letter of Recommendation Rating: ${formValue.lor}/5`, 25, yPosition);
          yPosition += 7;
        }
        if (formValue.cgpa) {
          doc.text(`CGPA: ${formValue.cgpa}/4`, 25, yPosition);
          yPosition += 7;
        }
        if (formValue.research !== undefined) {
          doc.text(`Research Experience: ${formValue.research === '1' ? 'Yes' : 'No'}`, 25, yPosition);
          yPosition += 7;
        }
        yPosition += 5;
      }
  ...
  generateProfileText(): string {
    ...
    if (formValue.greScore || formValue.toeflScore || formValue.universityRating || 
        formValue.sop || formValue.lor || formValue.cgpa || formValue.research) {
      profile += `\n=== Academic Scores ===\n`;
      if (formValue.greScore) profile += `GRE Score: ${formValue.greScore}\n`;
      if (formValue.toeflScore) profile += `TOEFL Score: ${formValue.toeflScore}\n`;
      if (formValue.universityRating) profile += `University Rating: ${formValue.universityRating}/5\n`;
      if (formValue.sop) profile += `Statement of Purpose Rating: ${formValue.sop}/5\n`;
      if (formValue.lor) profile += `Letter of Recommendation Rating: ${formValue.lor}/5\n`;
      if (formValue.cgpa) profile += `CGPA: ${formValue.cgpa}/4\n`;
      if (formValue.research !== undefined) profile += `Research Experience: ${formValue.research === '1' ? 'Yes' : 'No'}\n`;
    }
