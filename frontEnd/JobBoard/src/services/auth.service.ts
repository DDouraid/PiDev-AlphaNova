...
private userExists(userId: number): boolean {
  try {
    const url = `${this.authApiUrl}/users/${userId}`; // e.g., http://localhost:8088/api/auth/users/1
    // Simulating an HTTP request (replace with actual implementation if needed)
    console.log(`Checking if user exists at URL: ${url}`);
    // Assume a successful response for demonstration purposes
    return true;
  } catch (error) {
